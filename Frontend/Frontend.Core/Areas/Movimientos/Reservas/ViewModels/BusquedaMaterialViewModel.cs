...
{
    public class BusquedaMaterialViewModel : BaseViewModel, IBusquedaMaterialViewModel
    {
        ...
        public BusquedaMaterialViewModel(INavigationService navigationService, IDisplayAlertService displayAlertService,
        ...
        {
            await StartSpinner();
            var listMateriales = await reservaService.GetAllMaterialBy(EstadoMovimiento.Recibir, EstadoMovimiento.RechazadoSap);
        ...
        private async Task SearchMaterial()
        {
            ...
            {
                await StartSpinner();
                var reservas = await reservaService.GetAllBy(await lecturaQRService.GetLecturaQR(SearchValue));

                if (reservas.Count > 1)
                {
                    var answer = await displayAlertService.DisplayActionSheet("Reservas", "Cancelar", "", reservas.Select(x => x.Numero).ToArray());
