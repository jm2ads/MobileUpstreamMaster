...
{
    public class DevolucionMaterialViewModel : BaseViewModel, IDevolucionMaterialViewModel
    {
        ...
        public DevolucionMaterialViewModel(INavigationService navigationService, IDisplayAlertService displayAlertService,
            IReservaService reservaService, ISettingsService settingsService, ILecturaQRService lecturaQRService)
        {
        ...
        {
            await StartSpinner();
            var listMateriales = await reservaService.GetAllMaterialBy(TipoReserva.Devolucion, EstadoMovimiento.Recibir, EstadoMovimiento.RechazadoSap);
        ...
        private async Task SearchMaterial()
        {
            ...
            {
                await StartSpinner();
                var reservas = await reservaService.GetAllBy(await lecturaQRService.GetLecturaQR(SearchValue), TipoReserva.Devolucion);

                if (reservas.Count > 1)
                {
                    var answer = await displayAlertService.DisplayActionSheet("Devoluciones", "Cancelar", "", reservas.Select(x => x.Numero).ToArray());
        ...
        {
            navigationService.PushAsync<DevolucionMaterialView, CabeceraDevolucionView>(reserva);
        }
    }
}
