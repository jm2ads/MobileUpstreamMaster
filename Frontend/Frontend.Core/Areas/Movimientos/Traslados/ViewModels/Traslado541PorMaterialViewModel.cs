...
{
    public class Traslado541PorMaterialViewModel : BaseViewModel, ITraslado541PorMaterialViewModel
    {
        ...
        public Traslado541PorMaterialViewModel(INavigationService navigationService, IDisplayAlertService displayAlertService, IStockService stockService,
        ...
        private async void Init()
        {
            ...
            traslado = navigationService.GetNavigationParams<Traslado541PorMaterialView>() as Traslado;
        ...
        {
            await StartSpinner();
            var stockEspecialS = await stockEspecialService.GetByCodigo("S");
            var listMateriales = await stockService.GetAllMaterialBy(stockEspecialS.Id);
        ...
        private async Task SearchMaterial()
        {
            try
            {
                ...
                var stocks = await stockService.GetBy(idMaterial, lecturaQR.Lote, stockEspecialS.Id);
                if (stocks.Count != 1)
                {
                    var stockDictionary = CreateStockDictionary(stocks);
        ...
        private Dictionary<string, Stock> CreateStockDictionary(IList<Stock> stocks)
        {
            ...
            foreach (var stock in stocks)
            {
                ...
                var key = String.Format("Almacén:\t{0}\t|\tLote:\t{1}", almacen, stock.ClaseDeValoracion.Codigo);
        ...
        {
            navigationService.PushFromAsync<HomeView, ListaPosicionesTraslado541View>(traslado);
        }
    }
}
