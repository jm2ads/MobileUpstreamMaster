...
{
    public class Traslado542PorMaterialViewModel : BaseViewModel, ITraslado542PorMaterialViewModel
    {
        ...
        public Traslado542PorMaterialViewModel(INavigationService navigationService, IDisplayAlertService displayAlertService, IStockService stockService,
        ...
        private async void Init()
        {
            ...
            traslado = navigationService.GetNavigationParams<Traslado542PorMaterialView>() as Traslado;
        ...
        private async Task GetAllStocks()
        {
            ...
            var listMateriales = await stockService.GetAllMaterialBy(stockEspecial.Id);
        ...
        private async Task SearchMaterial()
        {
            try
            {
                ...
                var stocks = await stockService.GetBy(idMaterial, lecturaQR.Lote, stockEspecial.Id);
                if (stocks.Count != 1)
                {
        ...
        private Dictionary<string, Stock> CreateStockDictionary(IList<Stock> stocks)
        {
            ...
            foreach (var stock in stocks)
            {
                ...
                var almacen = stock.DetalleStockEspecial == null ? "   -   " : stock.DetalleStockEspecial.Detalle;
                var key = String.Format("Proveedor:\t{0}\t|\tLote:\t{1}", almacen, stock.ClaseDeValoracion.Codigo);
