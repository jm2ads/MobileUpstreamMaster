...
{
    public class Traslado309PorMaterialViewModel : BaseViewModel, ITraslado309PorMaterialViewModel
    {
        ...
        public Traslado309PorMaterialViewModel(INavigationService navigationService, IDisplayAlertService displayAlertService, IStockService stockService,
        ...
        private async void Init()
        {
            ...
            traslado = navigationService.GetNavigationParams<Traslado309PorMaterialView>() as Traslado;
        ...
        private async Task GetAllStocks()
        {
            ...
            var listMateriales = await stockService.GetAllMaterialExceptBy(stockEspecial.Id);
        ...
        private async Task SearchMaterial()
        {
            try
            {
                ...
                var stocks = await stockService.GetByAndExceptStockEspecial(idMaterial, lecturaQR.LoteId, stockEspecial.Id);
                if (stocks.Count != 1)
                {
        ...
        private async Task<Dictionary<string, Stock>> CreateStockDictionary(IList<Stock> stocks)
        {
            ...
            foreach (var stock in stocks)
            {
                ...
                string key;
                var stockEspecialQ = await stockEspecialService.GetByCodigo("Q");
                var stockEspecialK = await stockEspecialService.GetByCodigo("K");
                if (stock.DetalleStockEspecial.IdStockEspecial == stockEspecialQ.Id)
                {
                    key = String.Format("Almacén:\t{0}\t|\tLote:\t{1}\t|\tPEP:\t{2}", almacen, stock.ClaseDeValoracion.Codigo, stock.DetalleStockEspecial.Detalle);
                }
                else if (stock.DetalleStockEspecial.IdStockEspecial == stockEspecialK.Id)
                {
                    key = String.Format("Almacén:\t{0}\t|\tLote:\t{1}\t|\tProveedor:\t{2}", almacen, stock.ClaseDeValoracion.Codigo, stock.DetalleStockEspecial.Detalle);
                }
                else
                {
                    key = String.Format("Almacén:\t{0}\t|\tLote:\t{1}|\tStock:\t{2}", almacen, stock.ClaseDeValoracion.Codigo, stock.DetalleStockEspecial.StockEspecial.Codigo);
                }
